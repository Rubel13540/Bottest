...
async def admin_add_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"➕ Added Amount: `{amount} BDT`\n"
            f"💰 New Balance: `{new_bal:.2f} BDT`",
            parse_mode="Markdown"
        )
        ...
        await update.message.reply_text("❌ *সঠিক ফরম্যাট ব্যবহার করুন:*\n`/addbalance USER_ID AMOUNT`", parse_mode="Markdown")
...
async def admin_deduct_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"👤 User ID: `{target_user_id}`\n"
            f"➖ Deducted Amount: `{amount} BDT`\n"
            f"💰 New Balance: `{new_bal:.2f} BDT`",
            parse_mode="Markdown"
        )
    except (IndexError, ValueError):
        await update.message.reply_text("❌ *সঠিক ফরম্যাট ব্যবহার করুন:*\n`/deductbalance USER_ID AMOUNT`", parse_mode="Markdown")
    ...
    if step == "quantity":
        ...
        if response and "order" in response:
            ...
            await update.message.reply_text(
                ...
                f"🔗 Link: {link}\n"
                f"🔢 Quantity: `{quantity}`\n"
                f"💰 মোট কেটে নেওয়া হয়েছে: `{total_cost:.2f} BDT`",
                parse_mode="Markdown"
            )
        elif response and "error" in response:
            error_msg = response["error"]
            logger.error(f"SMM Provider Order Error: {error_msg}")
            await update.message.reply_text(
                f"⚠️ *অর্ডার প্লেস করা সম্ভব হয়নি!*\n\n"
                f"প্যানেলে পর্যাপ্ত ফান্ড নেই অথবা সার্ভিসে সাময়িক সমস্যা রয়েছে। দয়া করে অ্যাডমিনের সাথে যোগাযোগ করুন।",
                parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(
                "❌ API রেসপন্স পেতে ব্যর্থ হয়েছে। কিছুক্ষণ পর আবার চেষ্টা করুন।"
            )
...
def main():
    ...
    app.add_handler(CommandHandler("deductbalance", admin_deduct_balance))
