...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    menu_buttons = [
        "🚀 সার্ভিস ক্যাটালগ", "📍 অর্ডার ট্র্যাক করুন",
        "👤 প্রোফাইল ও ব্যালেন্স", "💳 ব্যালেন্স রিচার্জ",
        "💬 ২৪/৭ হেল্পলাইন", "📜 শর্তাবলী ও নিয়ম"
    ]
    ...
    if text == "🚀 সার্ভিস ক্যাটালগ":
        ...
        terms_text = (
            ...
            "   • ভুল লিংক বা প্রাইভেট অ্যাকাউন্টের লিংক দিলে অর্ডার সম্পন্ন হবে না এবং টাকা রিফান্ড দেওয়া হবে না।\n\n"
            "২. *অ্যাকাউন্ট পাবলিক রাখা:*\n"
            "   • সার্ভিস নেওয়ার সময় আপনার প্রোফাইল, পেজ বা পোস্ট অবশ্যই *Public* রাখতে হবে।\n\n"
            "৩. *ব্যালেন্স অ্যাড ও রিফান্ড:*\n"
            "   • বটে ব্যালেন্স অ্যাড করার পর কোনো অবস্থাতেই ক্যাশআউট বা রিফান্ড দেওয়া হবে না।\n"
            "   • জমা রাখা ব্যালেন্স শুধুমাত্র সার্ভিস ক্রয়ের কাজে ব্যবহার করতে পারবেন।\n\n"
            "৪. *পেমেন্ট প্রুফ:*\n"
            "   • বিকাশ বা নগদে টাকা পাঠানোর পর TrxID এবং স্ক্রিনশট হোয়াটসঅ্যাপ সাপোর্টে পাঠাতে হবে।\n\n"
            "৫. *একাধিক অর্ডার:*\n"
            "   • একই লিংকে একই সাথে একাধিক অর্ডার দেবেন না। একটি অর্ডার শেষ হওয়ার পর পরবর্তী অর্ডারটি দিন।\n\n"
...
async def process_order_steps(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "quantity":
        ...
        # Automatic exact price calculation based on user input quantity
        rate_per_1000 = calculate_customer_price(service.get("rate", 0))
        total_cost = (Decimal(quantity) / Decimal("1000")) * rate_per_1000

        await update.message.reply_text(
            f"✅ *অর্ডার হিসাব বিবরণী*\n\n"
            f"📌 Service: {service.get('name')}\n"
            f"🔗 Link: {context.user_data.get('order_link')}\n"
            f"🔢 Quantity: `{quantity}`\n"
            f"💰 *আপনার এই পরিমাণের জন্য মোট খরচ: `{total_cost:.2f} BDT`*",
            parse_mode="Markdown"
        )
        context.user_data.clear()
