...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["🚀 সার্ভিস ক্যাটালগ", "📊 লাইভ সার্ভিস রেট"]:
        ...
        keyboard = []
        for cat in categories[:25]:  # Show top 25 categories
            keyboard.append([InlineKeyboardButton(f"📁 {cat[:35]}", callback_data=f"cat_{cat[:30]}")])

        await update.message.reply_text(
            "📁 **পছন্দের ক্যাটাগরি বেছে নিন:**\n\n"
            "নিচের অপশনগুলো থেকে আপনার প্রয়োজনীয় প্ল্যাটফর্ম বা সার্ভিস ক্যাটাগরি নির্বাচন করুন 👇", 
...
async def category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    for s in services:
        cat_name = s.get('category', 'General')[:30]
        if cat_name == selected_cat:
            s_id = s.get('service')
            usd_rate = float(s.get('rate', 0.0))
            bdt_rate = usd_rate * USD_TO_BDT
            name = s.get('name', 'Service')[:30]
            btn_text = f"🔥 {name} - ৳{bdt_rate:.2f}"
            keyboard.append([InlineKeyboardButton(btn_text, callback_data=f"select_srv_{s_id}_{bdt_rate:.4f}")])

    if not keyboard:
        await query.message.edit_text("⚠️ এই ক্যাটাগরিতে কোনো সার্ভিস পাওয়া যায়নি।")
        return
    ...
    await query.message.edit_text(
        f"💎 **ক্যাটাগরি: {selected_cat}**\n\nঅর্ডার করতে পছন্দের সার্ভিসটিতে ক্লিক করুন 👇",
        reply_markup=InlineKeyboardMarkup(keyboard[:30])
    )
...
async def select_service_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['order_rate'] = float(bdt_rate)

    await query.message.edit_text(
        f"🛒 **নতুন অর্ডার নির্বাচন**\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"🆔 **সার্ভিস আইডি:** `{s_id}`\n"
        f"💵 **মূল্য (প্রতি ১০০০):** ৳{float(bdt_rate):.2f} BDT\n"
        f"━━━━━━━━━━━━━━━━━━━\n\n"
