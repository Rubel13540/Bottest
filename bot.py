...
def main_reply_keyboard():
    keyboard = [
        ...
        [KeyboardButton("👤 মাই প্রোফাইল"), KeyboardButton("💳 টাকা জমা দিন (রিচার্জ)")],
        [KeyboardButton("💬 কাস্টমার সাপোর্ট"), KeyboardButton("📜 শর্তাবলী ও নিয়ম")]
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_banned:
        await update.message.reply_text("❌ আপনার একাউন্টটি সাময়িকভাবে স্থগিত করা হয়েছে। সাপোর্টে কথা বলুন।")
    ...
    text = (
        ...
        f"👇 **অর্ডার করতে বা সার্ভিস দেখতে নিচের মেনু ব্যবহার করুন:**"
    )
    ...
    if update.message:
        await update.message.reply_text("✨ সেবা পেতে নিচের অপশন বেছে নিন:", reply_markup=main_reply_keyboard())
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["🚀 সার্ভিস ক্যাটালগ"]:
        ...
        for cat in categories[:20]:
            ...
            emoji = "📁"
            if "YouTube" in cat: emoji = "🔴"
            elif "Facebook" in cat: emoji = "🔵"
            elif "TikTok" in cat: emoji = "🎵"
            elif "Instagram" in cat: emoji = "📸"
            elif "Telegram" in cat: emoji = "✈️"

            keyboard.append([InlineKeyboardButton(f"{emoji} {bangla_cat[:32]}", callback_data=f"cat_{cat[:30]}")])

        await update.message.reply_text("🏷️ **পছন্দের প্ল্যাটফর্ম নির্বাচন করুন:**", reply_markup=InlineKeyboardMarkup(keyboard))

    elif text == "👤 মাই প্রোফাইল":
    ...
    elif text == "💳 টাকা জমা দিন (রিচার্জ)":
        await update.message.reply_text(
...
async def category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in services:
        if s.get('category', 'General')[:30] == selected_cat:
            ...
            bangla_name = translate_to_bangla(s.get('name', 'Service'))[:28]
            keyboard.append([InlineKeyboardButton(f"🔥 {bangla_name} - ৳{bdt_rate:.2f}", callback_data=f"select_srv_{s_id}_{bdt_rate:.4f}")])

    keyboard.append([InlineKeyboardButton("🔙 প্রধান মেনু", callback_data='back_to_main')])
...
async def order_get_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔢 **ধাপ ২/২:** পরিমাণ (Quantity) লিখুন:")
    return ORDER_QTY
...
async def order_get_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if api_res and 'order' in api_res:
            ...
            await update.message.reply_text(f"⚠️ অর্ডার সম্পন্ন হয়নি। কারণ: {err}")
...
async def start_track_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔎 আপনার **Order ID** নম্বরটি লিখুন:")
...
async def process_track_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if res and 'status' in res:
        ...
        await update.message.reply_text("⚠️ সঠিক Order ID পাওয়া যায়নি বা সার্ভার ত্রুটি।")
    return ConversationHandler.END
...
if __name__ == '__main__':
    ...
    if not token:
        ...
        track_conv = ConversationHandler(
            entry_points=[MessageHandler(filters.Regex('^📍 অর্ডার ট্র্যাক করুন$'), start_track_order)],
            states={
                TRACK_ORDER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_track_order)],
            },
            fallbacks=[], per_message=False
        )
        ...
        admin_conv = ConversationHandler(
            entry_points=[
                CallbackQueryHandler(adm_start_addbal, pattern='^adm_addbal$'),
                CallbackQueryHandler(adm_start_broadcast, pattern='^adm_broadcast$')
            ],
            states={
                ...
                BROADCAST_MSG: [MessageHandler(filters.TEXT & ~filters.COMMAND, adm_send_broadcast)],
            },
            fallbacks=[], per_message=False
        )
